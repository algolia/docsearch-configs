new Crawler({
  ...
  startUrls: [
    ...
    "http://movevis.org/",
    "http://movevis.org/reference",
    "http://movevis.org/articles",
  ],
  renderJavaScript: false,
  sitemaps: ["http://movevis.org/sitemap.xml"],
  exclusionPatterns: [
    "**/reference/",
  ...
  discoveryPatterns: ["http://movevis.org/**"],
  schedule: "at 10:30 on Thursday",
  actions: [
    {
      indexName: "movevis",
      pathsToMatch: ["http://movevis.org/index.html**/**"],
    ...
    {
      indexName: "movevis",
      pathsToMatch: ["http://movevis.org/reference**/**"],
      recordExtractor: ({ $, helpers }) => {
        // Removing DOM elements we don't want to crawl
        const toRemove = ".dont-index";
        $(toRemove).remove();
    ...
    {
      indexName: "movevis",
      pathsToMatch: ["http://movevis.org/articles**/**"],
      recordExtractor: ({ $, helpers }) => {
        // Removing DOM elements we don't want to crawl
        const toRemove = ".dont-index";
        $(toRemove).remove();
