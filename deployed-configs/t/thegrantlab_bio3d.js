new Crawler({
  ...
  sitemaps: ["http://thegrantlab.org/bio3d/sitemap.xml"],
  exclusionPatterns: [
    "**/reference/",
  ...
  discoveryPatterns: ["http://thegrantlab.org/**"],
  schedule: "at 19:10 on Friday",
  actions: [
    {
      indexName: "thegrantlab_bio3d",
      pathsToMatch: ["http://thegrantlab.org/bio3d/index.html**/**"],
    ...
    {
      indexName: "thegrantlab_bio3d",
      pathsToMatch: ["http://thegrantlab.org/bio3d/reference**/**"],
      recordExtractor: ({ $, helpers }) => {
        // Removing DOM elements we don't want to crawl
        const toRemove = ".dont-index";
        $(toRemove).remove();
    ...
    {
      indexName: "thegrantlab_bio3d",
      pathsToMatch: ["http://thegrantlab.org/bio3d/articles**/**"],
      recordExtractor: ({ $, helpers }) => {
        // Removing DOM elements we don't want to crawl
        const toRemove = ".dont-index";
        $(toRemove).remove();
